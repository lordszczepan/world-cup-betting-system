const roundOf32SlotOrder = ["1A", "1B", "1D", "1E", "1G", "1I", "1K", "1L"] as const

type RoundOf32Slot = (typeof roundOf32SlotOrder)[number]

type AnnexCAssignment = Record<RoundOf32Slot, string>

const rawAnnexCMatrix = `1 3E 3J 3I 3F 3H 3G 3L 3K
...
export function getAnnexCAssignment(qualifiedThirdGroups: string[]) {
  if (qualifiedThirdGroups.length !== 8) {
    return null
  }

  const normalizedKey = qualifiedThirdGroups
    .map((group) => group.trim().toUpperCase())
    .sort()
    .join('')

  for (const line of rawAnnexCMatrix.trim().split('\n')) {
    const [, ...slots] = line.trim().split(/\s+/)
    const assignment = Object.fromEntries(roundOf32SlotOrder.map((slot, index) => [slot, slots[index]])) as AnnexCAssignment
    const assignmentKey = slots
      .map((slot) => slot.replace('3', ''))
      .sort()
      .join('')

    if (assignmentKey === normalizedKey) {
      return assignment
    }
  }
